...
//-----------------------------------------------------------------------
/* Ceci est le code choisit pour d�sign�s chacune des fontions �lementaires
ABS    <=>   'B'   ?
COS    <=>   'C'
SIN    <=>   'S'
TAN    <=>   'T'
...
/* Proc�dure de propagation de contraintes binaires sur les
  intervalles, cette proc�dure est charg�e de faire appel � une
...

{
	...
	if (code == '_')   { Cmoins(Y, X, sens);   return; }
	if (code == '=')   { Cegal(Y, X, sens);    return; }
	if (code == 'B')   { Cabs(Y, X, sens);     return; }
	if (code == 'D')   { Crect(Y, X, sens);    return; }
	if (code == 's')   { Csinc(Y, X, sens);     return; }
	//if (code=='B')   {Cpower(Y,X,2,sens); return;}
}
//----------------------------------------------------------------------
/*void Contract0 (char code, interval& Y, interval& X, int n, int sens)
   Proc�dure de propagation de contraintes concernant la puissance enti�re
   sur les intervalles : Y=X^n, cette proc�dure est charg�e de faire appel �
   l'op�rateur �l�mentaire Power.
...
/* Proc�dure de propagation de contraintes ternaires sur les
  intervalles, cette proc�dure est charg�e de faire appel aux
  fonctions ternaires.
  exemple:  Z=Y+X         =>  Contract0('+',Z,Y,X,1)
  Y=Z-X; X=Z-Y  =>  Contract0('+',Z,Y,X,-1) */
{
	if (code == '+')    { Cplus(Z, Y, X, sens);    return; }
	if (code == '-')    { Cmoins(Z, Y, X, sens);   return; }
	if (code == '*')    { Cprod(Z, Y, X, sens);    return; }
...
//----------------------------------------------------------------------
//modifs???
// RETURN LA DIFFERENCE ENTRE 2 intervalles x0, x1 et place le r�sultat dans c0 et c1.
